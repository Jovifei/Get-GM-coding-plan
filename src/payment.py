    ...
    async def handle_payment(self, purchase_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            with diagnostic_step(logger, "尝试余额支付"):
                balance_paid = await self._try_balance_pay()
            if balance_paid:
                result["success"] = True
            ...
            with diagnostic_step(logger, "获取扫码支付信息"):
                qr_result = await self._get_qrcode_info()
            if qr_result:
    ...
    async def _try_balance_pay(self) -> bool:
        ...
        try:
            ...
            if await pay_btn.is_visible(timeout=3000):
                ...
                # 检查是否成功
                if await self._check_payment_success():
                    return True

                # 检查是否需要扫码
                if await self._check_qrcode_required():
        ...
        try:
            ...
            for selector in qr_selectors:
                try:
                    ...
                    if await qr_img.is_visible(timeout=3000):
                        result["qrcode"] = await qr_img.get_attribute("src")
                        break
                except Exception:
                    continue
            ...
            for selector in link_selectors:
                try:
                    ...
                    if await link.is_visible(timeout=2000):
                        result["url"] = await link.get_attribute("href")
                        break
                except Exception:
                    continue
